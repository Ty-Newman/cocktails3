...
-- Matches migrations through 20250416100000_bar_open_email_notifications.sql
...
create table public.profiles (
    ...
    notify_bar_open_email boolean not null default true,
    created_at timestamptz not null default timezone('utc'::text, now()),
    updated_at timestamptz not null default timezone('utc'::text, now())
);
...
-- -----------------------------------------------------------------------------
-- Bar open + email recipient helper (Edge Function + Resend)
-- -----------------------------------------------------------------------------
